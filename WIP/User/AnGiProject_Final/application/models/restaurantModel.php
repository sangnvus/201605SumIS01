...
class RestaurantModel extends CI_Model
{
    ...
    function Search($offset=0, $limit=20, &$count, &$categories, &$districts, $cat, $district, $keyword){
        ...
        if ($keyword != ''){
            ...
            /*
            $where .= ' and MATCH( ';
            $where .= $this->table.'.nameRe, ';    
            $where .= $this->table.'.descriptionRes ';    
            $where .= ' ) ';    
            $where .= ' AGAINST (\''.$keyword.'\' IN NATURAL LANGUAGE MODE';    
            $where .= ' ) ';
            */  
        }
        if ($cat > 0){
